    ...
    try:
        ...
        if message=='ok':
            ...
            if (items_size != 0):
                ...
                param = (lon, lat, str(items_size), '2017-08-14 21:00:00', str(items_size))
            ...
            for item in items:
                db = MySQLdb.connect(host='localhost', passwd='123aaaaaa', user='root', db='ofo', charset='utf8')
                cursor = db.cursor()
                sql = "insert into putuo_mobike_address2 (distX,distY,bikeIds,source,save_time) values (%s,%s,%s,%s,'2017-08-14 21:00:00') on duplicate key update distY = %s"
